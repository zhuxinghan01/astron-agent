...
class TestHttpRun:
    ...
    def valid_http_run_request(self):
        ...
        return {
            ...
            "payload": {
                "message": {
                    "header": base64.b64encode(json.dumps({"Authorization": "Bearer token"}).encode()).decode(),
                    "query": base64.b64encode(json.dumps({"q": "test query"}).encode()).decode(),
                    "path": base64.b64encode(json.dumps({"id": "123"}).encode()).decode(),
                    "body": base64.b64encode(json.dumps({"data": "test"}).encode()).decode()
                }
            }
        }
    ...
    async def test_http_run_validation_error(self, mock_api_validate, mock_span_class):
        ...
        mock_request = Mock()
        mock_request.model_dump.return_value = {"header": {}, "parameter": {}, "payload": {}}
    ...
    async def test_http_run_with_authentication(self, mock_get_redis, mock_http_run_class,
        ...
        mock_redis.get.return_value = {
            "authentication": {
                "apiKey": {"X-API-Key": "secret_api_key"}
            }
        }
