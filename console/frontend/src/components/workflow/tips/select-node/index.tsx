...
export default function Select({
  ...
  const copyNodes = async () => {
    ...
    cloneLastSelection.nodes = cloneLastSelection.nodes?.filter(node => node.type !== '开始节点' && node.type !== '结束节点')
    try {
  ...
  return (
    <div className='fixed top-[100px] left-[50%] translate-x-[-50%] z-50 flex items-center gap-2'>
      ...
      <div className='px-4 py-2 rounded-md bg-[#275EFF] text-white cursor-pointer'
        onClick={() => copyNodes()}
      >
        复制
      </div>
